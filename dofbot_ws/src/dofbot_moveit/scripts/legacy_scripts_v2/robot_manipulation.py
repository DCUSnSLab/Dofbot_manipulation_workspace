...
class RobotManipulation:
    ...
    def marker_array_callback(self, msg: MarkerArray):
        if msg.markers:
            ...
            if msg.markers[0].pose.position.x == 0:
                xy_obj = [0.1, msg.markers[0].pose.position.y]
                xz_obj = [0.1, msg.markers[0].pose.position.z]
            else:
                xy_obj = [msg.markers[0].pose.position.x, msg.markers[0].pose.position.y]
                xz_obj = [msg.markers[0].pose.position.x, msg.markers[0].pose.position.z]
            ...
            # 실물 서보모터를 각각 회전시킵니다.
            self.sbus.Arm_serial_servo_write(1, degree_of_servo[0], 100)
            self.sbus.Arm_serial_servo_write(2, degree_of_servo[1], 100)
        else: rospy.logwarn("Markers list is empty ! !")
    ...
    def print_each_marker_info(self, turn_on: bool, marker_ary: MarkerArray):
        if turn_on:
            print("*---*---*---*---*---*---*")
            for marker in marker_ary.markers:
                # 마커의 정보를 출력합니다.
                ...
                print("\t> ({0:.2f}, {1:.2f}, {2:.2f})".format(marker.pose.position.x, marker.pose.position.y, marker.pose.position.z))
                print("-----")
            print()
            print()
