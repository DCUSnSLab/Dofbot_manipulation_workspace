...
class DofbotApp(QWidget):
    ...
    def __init__(self):
        ...
        self.obj_layers = {0:QVBoxLayout(), 1:QVBoxLayout(), 2:QVBoxLayout()}
        self.cbs = []
        self.info_boxes = []
        self.labels_obj = []
        self.coord = []
        ...
        ## ROS service로 발행할 메시지를 구성하는 마커들을 선언합니다.
        self.marker_array = MarkerArray()
        self.m1 = Marker()
        self.m2 = Marker()
        self.m3 = Marker()
        self.marker_array.append(self.m1)
        self.marker_array.append(self.m2)
        self.marker_array.append(self.m3)
    ...
    def set_coords(self):
        ...
        :return: None
        '''
    def make_random_start_point(self, marker: Marker):
        '''
        해당 마커 객체에 임의의 3차원 좌표와 색상값을 추가합니다.
        :param marker: 마커 객체 자체
        :return: None
        '''
        rnd_x = int(random.uniform(-250, 250))
        rnd_y = int(random.uniform(-250, 250))
        rnd_z = int(random.uniform(0, 250))
        rnd_coord = [rnd_x, rnd_y, rnd_z]

        marker.color.a = 1.0
        marker.scale.x = 0.5
        rnd_r = float(random.uniform(0, 1))
        rnd_g = float(random.uniform(0, 1))
        rnd_b = float(random.uniform(0, 1))
        marker.color.r = rnd_r
        marker.color.g = rnd_g
        marker.color.b = rnd_b

        for i in range(3):
            marker.points.append(rnd_coord[i])
    def set_obj_layers_info(self):
        '''
        객체 전체를 정의합니다.
        :return: None
        '''
        for i in range(3):
            self.marker_array[i].header.frame_id = '-'
            self.marker_array[i].id = i
            self.marker_array[i].type = Marker.POINTS
            self.marker_array[i].action = Marker.ADD
            self.make_random_start_point(self.marker_array[i])
    def set_group_box(self):
        ...
        for i in range(3):
            ...
            self.labels_obj.append(QLabel('3차원 좌표 : '))
            self.labels_obj[i].setAlignment(Qt.AlignRight | Qt.AlignVCenter)
            ...
            self.info_boxes[i].addWidget(self.labels_obj[i])
            ...
            self.group_box_vbox.addLayout(self.obj_layers[i])
